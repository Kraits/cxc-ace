...
export async function POST(req: NextRequest) {
  try {
    ...
    const safeSubject = subject ? String(subject).replace(/[^\w\s\-()&]/g, '').slice(0, 100) : '';
    const safeTopic = topic ? String(topic).replace(/[^\w\s\-()&]/g, '').slice(0, 100) : '';
